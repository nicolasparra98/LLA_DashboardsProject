...
----- Panama
,FMC_CWP_Network  AS (
SELECT month, Opco, Tech as Network, count(distinct Active_Base) as Active_Base, sum(Vol_Churners) as Total_Voluntary_Churners, sum(Early_Vol_Churners) as Early_Voluntary_Churners, sum(Mid_Vol_Churners) as Mid_Voluntary_Churners, sum(Late_Vol_Churners) as Late_Voluntary_Churners, sum(Invol_Churners) as Total_Involuntary_Churners, sum(Early_Invol_Churners) as Early_Involuntary_Churners, sum(Mid_Invol_Churners) as Mid_Involuntary_Churners, sum(Late_Invol_Churners) as Late_Involuntary_Churners
FROM FMC_Table_CWP 
...
,FMC_CWP_Overall  AS (
SELECT month, Opco, 'OVERALL' as Network, count(distinct Active_Base) as Active_Base, sum(Vol_Churners) as Total_Voluntary_Churners, sum(Early_Vol_Churners) as Early_Voluntary_Churners, sum(Mid_Vol_Churners) as Mid_Voluntary_Churners, sum(Late_Vol_Churners) as Late_Voluntary_Churners, sum(Invol_Churners) as Total_Involuntary_Churners, sum(Early_Invol_Churners) as Early_Involuntary_Churners, sum(Mid_Invol_Churners) as Mid_Involuntary_Churners, sum(Late_Invol_Churners) as Late_Involuntary_Churners
FROM FMC_Table_CWP 
WHERE Tech IN ('COPPER','FTTH','HFC')
GROUP BY 2,1,3
ORDER BY 2,1,3
...
----- Results
,Result_Table as(
SELECT  A.*, B.Active_Base as Customers, B.Total_Voluntary_Churners, B.Early_Voluntary_Churners, B.Mid_Voluntary_Churners, B.Late_Voluntary_Churners, B.Total_Involuntary_Churners, B.Early_Involuntary_Churners, B.Mid_Involuntary_Churners, B.Late_Involuntary_Churners
