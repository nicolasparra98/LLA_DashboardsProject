...
PD_BB_PROD_CD, pd_tv_prod_cd, PD_VO_PROD_CD, pd_mix_nm,pd_mix_cd,date(dt) as dt,exclude,inv_churn_flg,oldest_unpaid_bill_dt
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2022-11-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2022-11-01')
GROUP BY 1,2,3,4
)
...
,Fix_B_Date,FixedAccount_BOM,Fixed_B_Phone,B_Overdue,Fixed_B_MaxStart,B_FixedTenure,B_Fixed_MRC,B_TechFlag,B_NumRGUs,B_MixName_Adj,B_MixCode_Adj,B_BB,B_TV,B_VO,B_bbCode,B_tvCode,B_voCode,B_Hard_FMC_Flag,Fix_E_Date,Fixed_E_Phone,E_Overdue,Fixed_E_MaxStart,E_FixedTenure,E_Fixed_MRC,E_TechFlag
,case when fixedchurntype = '2. Fixed Involuntary Churner' --and fixedmainmovement in ('1.SameRGUs','3.Downsell') 
...
where fixedmonth=date('2022-11-01')

/*
select fixedmonth, fixedchurntype, fixedmainmovement, b_techflag, F_ActiveBOM,F_ActiveEOM,count(distinct fixedaccount) as cuentas , sum(b_numrgus) as BOM_RGUS, sum(e_numrgus) as EOM_RGUS
FROM FinalChurnFlag_SO
where fixedmonth=date('2022-11-01')
