--CREATE TABLE IF NOT EXISTS "lla_cco_int_stg"."cwp_mobile_oct25_NP_mes7_v3" AS
...
--AND date(dt) between (DATE('2022-07-01') + interval '1' MONTH - interval '1' DAY - interval '3' MONTH) AND  (DATE('2022-07-01') + interval '1' MONTH - interval '1' DAY + interval '3' MONTH)
GROUP BY DT,2,3,4,ACCOUNTNAME,7,8,9,10
)

--Select *from mobileusefulfields where Mobileaccount = '1762634' and MobileMonth = date('2022-07-01')
...
--Select sum (Numrgus) from numberrgusperuser where MobileMonth = date('2022-07-01') --4.751.968
--Select count(*) from numberrgusperuser where MobileMonth = date('2022-07-01') -- 5.125.245
--Select * from NumberRGUsPerUser  where MobileMonth = date('2022-07-01') order by NumRgus desc
,AverageMRC_User AS(
...
,CASE WHEN (E_MobileRGUs - B_MobileRGUs) = 0 THEN '1.SameRGUs' 
      ...
      WHEN (B_MobileRGUs IS NULL AND E_MobileRGUs > 0 AND DATE_TRUNC('MONTH', Mobile_E_MaxStart) = DATE('2022-07-01')) THEN '4.New Customer'
      WHEN (B_MobileRGUs IS NULL AND E_MobileRGUs > 0 AND DATE_TRUNC('MONTH', Mobile_E_MaxStart) <> DATE('2022-07-01')) THEN '5.Come Back to Life'
...
WHERE dt='2022-06-01' AND RGU_DRC = 'Y'
)
,DerecognitionBaseMonth AS(
SELECT '2022-07-01' AS DRCMonth, CAST(ACCOUNTNO AS VARCHAR(50)) AS AccountNo
FROM "lla_cco_int_ext_prod"."cwp_mov_ext_derecognition"
WHERE dt='2022-07-01' AND RGU_DRC = 'Y'
)
,MonthCleaning AS(
SELECT m.*
,CASE WHEN (j.AccountNo IS NOT NULL and MobileMainMovement in ('6.Null last day','1.SameRGUs')) OR (j.AccountNo IS NOT NULL and d.AccountNo IS NOT NULL) 
...
,CASE WHEN RejoinerMonth>=date('2022-07-01') AND RejoinerMonth<=DATE_ADD('MONTH',1,date('2022-07-01')) THEN 1 ELSE 0 END AS Mobile_PRMonth
...
SELECT DISTINCT Mobile_Month,RejoinerPopFlag,Mobile_PRMonth,Mobile_Account,date('2022-07-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Mobile_PRMonth=1
AND Mobile_Month<>date('2022-07-01')
GROUP BY 1,2,3,4
)
...
where mobile_month=date('2022-07-01') and drc=0 
