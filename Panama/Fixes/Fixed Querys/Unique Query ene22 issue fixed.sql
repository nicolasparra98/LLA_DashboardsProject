...
--CREATE TABLE IF NOT EXISTS "lla_cco_int_stg"."FMC_prueba_NP_oct" AS
...
WHERE telefonia='Pospago' AND "unidad de negocio"='1. B2C'
 AND DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2023-01-01') or DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-12-01')
)
...
from (select *,
        ...
        and date(dt) between (DATE('2023-01-01') + interval '1' MONTH - interval '1' DAY - interval '2' MONTH) AND  (DATE('2023-01-01') + interval '1' MONTH - interval '1' DAY)) 
)
...
WHERE act_cust_typ_nm = 'Residencial'
        AND DATE_TRUNC('month',DATE(dt)) BETWEEN (DATE('2023-01-01') - interval '3' month) AND (DATE('2023-01-01') - interval '1' month)
GROUP BY 1,2
ORDER BY 1,2
)
...
WHERE PD_MIX_CD<>'0P' and act_cust_typ_nm = 'Residencial'
        ...
        AND DATE_TRUNC('month',CAST(dt AS DATE)) = DATE('2023-01-01')
GROUP BY 2,1
)
...
WHERE PD_MIX_CD<>'0P'AND act_cust_typ_nm = 'Residencial' 
and date(a.dt) between (DATE('2023-01-01') + interval '1' MONTH - interval '1' DAY - interval '2' MONTH) AND  (DATE('2023-01-01') + interval '1' MONTH - interval '1' DAY)
)
...
WHERE order_type in ('INSTALLATION','SELF INSTALL') AND ACCOUNT_TYPE='R' AND ORDER_STATUS='COMPLETED'
        AND DATE_TRUNC('MONTH',CAST(completed_date AS DATE)) = DATE('2023-01-01') -- interval '1' month and (SELECT month_analysis FROM parameters) --+ interval '1' month
)
...
SELECT  date_trunc('MONTH',DATE('2023-01-04')) AS month
        ,FixedAccount AS CandidateAccount
FROM FixedUsefulFields
WHERE date(dt) = DATE('2023-01-04')
    AND fi_outst_age >= 94 
...
,CASE WHEN NumBB = 1 AND NumTV = 0 AND NumVO = 0 THEN 'BO'
    ...
    and FixedAccount not in (select distinct CandidateAccount from Candidates_IssueDic22)
    and exclude<>1
)
...
,CASE WHEN RejoinerMonth>=date('2023-01-01') AND RejoinerMonth<=DATE_ADD('MONTH',1,date('2023-01-01')) THEN 1 ELSE 0 END AS Fixed_PRMonth
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2023-01-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2023-01-01')
GROUP BY 1,2,3,4
)
...
where fixedmonth=date('2023-01-01') and (f_activeBOM + f_activeEOM >= 1)
