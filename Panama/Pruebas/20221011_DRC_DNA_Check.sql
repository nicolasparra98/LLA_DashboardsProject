...
,MonthCleaning AS(
SELECT m.*
,CASE WHEN j.AccountNo IS NOT NULL THEN 1 ELSE 0 END AS DRC --CONDICION MODIFICADA
...
SELECT distinct mobile_month, mobile_account--, Mobile_ActiveBOM, Mobile_ActiveEOM
FROM MonthMobileRejoiners
WHERE mobile_month=date('2022-09-01') and drc=1
GROUP BY 1,2--,3,4
ORDER BY 1,2--,3,4
)

,DRC_septiembre as (
select *, 1 as DRC_septiembre
from "lla_cco_int_ext_prod"."cwp_mov_ext_derecognition" where RGU_DRC = 'Y' and date(dt)=date('2022-09-01')
)

,DRC_agosto as (
select *, 1 as DRC_agosto
from "lla_cco_int_ext_prod"."cwp_mov_ext_derecognition" where RGU_DRC = 'Y' and date(dt)=date('2022-08-01')
)

,join_derec as (
select C.*, D.DRC_septiembre
from (  select A.mobile_account,--A.Mobile_ActiveBOM, A.Mobile_ActiveEOM, 
B.DRC_agosto
        from Cuentas_A_Evaluar A left join DRC_agosto B on A.mobile_account = CAST(B.ACCOUNTNO AS VARCHAR(50))) C left join DRC_septiembre D
        on C.mobile_account = CAST(D.ACCOUNTNO AS VARCHAR(50))
)

,join_DNA_BOM AS (
SELECT DISTINCT A.mobile_account, CASE WHEN B.ACCOUNTNO IS NULL THEN 0 ELSE 1 END AS DNA_ActiveBOM_sep, B.ACCOUNT_STATUS
from Cuentas_A_Evaluar A left join (select * from "db-analytics-prod"."tbl_postpaid_cwp" where date(DT)=date('2022-08-31')) B 
on A.mobile_account = B.ACCOUNTNO
)

,join_DNA_EOM AS (
SELECT DISTINCT A.mobile_account, CASE WHEN B.ACCOUNTNO IS NULL THEN 0 ELSE 1 END AS DNA_ActiveEOM_sep, B.ACCOUNT_STATUS
from Cuentas_A_Evaluar A left join (select * from "db-analytics-prod"."tbl_postpaid_cwp" where date(DT)=date('2022-09-30')) B 
on A.mobile_account = B.ACCOUNTNO
)

,join_final as (
select C.*, D.DNA_ActiveEOM_sep, D.ACCOUNT_STATUS as status_EOM
from
(select A.*, B.DNA_ActiveBOM_sep, B.ACCOUNT_STATUS as status_BOM
from join_derec A left join join_DNA_BOM B on A.mobile_account=B.mobile_account
) C left join join_DNA_EOM D on C.mobile_account=D.mobile_account
)

--select count(distinct mobile_account), sum(DRC_agosto), sum(DRC_septiembre), sum(Mobile_ActiveBOM), sum(Mobile_ActiveEOM) from join_final

select distinct status_BOM as status_DNA_BOM_sep, status_EOM as status_DNA_EOM_sep, count(distinct mobile_account) as cuentas, sum(DRC_agosto) as DRC_active_agosto, sum(DRC_septiembre) as DRC_active_septiembre from join_final group by 1,2 order by 1,2
