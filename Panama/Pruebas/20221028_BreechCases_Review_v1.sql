...
from resultados --where month=date(dt)
) group by 1,2
)

--SELECT DISTINCT month, tech,unique_longinstall,unique_sales 
--FROM Sprint3_KPIs
--order by 1,2

select distinct month, Late_inst_flag, New_customer, count(distinct F_LongInstallFlag), count(distinct finalaccount) from FullTable_KPIsFlags 
where month = date('2022-09-01') and Late_inst_flag=1 and New_customer=1
