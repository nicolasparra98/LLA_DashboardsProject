...
,Sample_Damagenn11 as (
select 
distinct mobile_account
--distinct bandera1, BOM_Active_junio, EOM_Active_junio, BOM_Active_julio, EOM_Active_julio, ChurnerTypeJunio
--,count (distinct mobile_account) 
from join_estatus 
where bandera1='Damage nn11'
...
,Cruce_DRC as (
select distinct dt, count(distinct accountno) from Sample_Damagenn11 A left join "lla_cco_int_ext_prod"."cwp_mov_ext_derecognition" B on A.mobile_account=CAST(b.ACCOUNTNO AS VARCHAR(50)) 
where RGU_DRC = 'Y'
group by 1
order by 1
)

select *--distinct DT, count(distinct MobileAccount) 
from Sample_Damagenn11 A left join MobileUsefulFields B on A.mobile_account=B.MobileAccount
--where DT in (date('2022-04-30'),date('2022-05-31'),date('2022-06-30'),date('2022-07-31'),date('2022-08-31'))
where DT>date('2022-04-01')
--group by 1
order by mobile_account, DT
