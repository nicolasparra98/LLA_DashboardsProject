...
WHERE telefonia='Pospago' AND "unidad de negocio"='1. B2C'
 AND DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-10-01') or DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-09-01')
)
,lag_dna as (
...
WHERE act_cust_typ_nm = 'Residencial'
        AND DATE_TRUNC('month',DATE(dt)) BETWEEN (DATE('2022-10-01') - interval '3' month) AND (DATE('2022-10-01') - interval '1' month)
GROUP BY 1,2
ORDER BY 1,2
)
...
WHERE PD_MIX_CD<>'0P' and act_cust_typ_nm = 'Residencial'
        ...
        AND DATE_TRUNC('month',CAST(dt AS DATE)) = DATE('2022-10-01')
GROUP BY 2,1
)
...
WHERE PD_MIX_CD<>'0P'AND act_cust_typ_nm = 'Residencial' 
and date(a.dt) between (DATE('2022-10-01') + interval '1' MONTH - interval '1' DAY - interval '2' MONTH) AND  (DATE('2022-10-01') + interval '1' MONTH - interval '1' DAY)
)
...
WHERE order_type in ('INSTALLATION','SELF INSTALL') AND ACCOUNT_TYPE='R' AND ORDER_STATUS='COMPLETED'
        AND DATE_TRUNC('MONTH',CAST(completed_date AS DATE)) = DATE('2022-10-01') -- interval '1' month and (SELECT month_analysis FROM parameters) --+ interval '1' month
)

,FixedUsefulFields as(
select A.* 
        --,if(maxstart is not null,maxstart,if(new_customer = 1,date(B.completed_date),null)) as maxstartadj
...
,CASE WHEN NumBB = 1 AND NumTV = 0 AND NumVO = 0 THEN 'BO'
    ...
    FROM FixedUsefulFields f LEFT JOIN HardBundleFlag h ON f.fixedaccount = h.ACT_ACCT_CD AND date(f.DT) = date(h.DT)
    WHERE DATE(f.dt) = --date('2023-01-29') --
    date_trunc('MONTH', DATE(f.dt)) + interval '1' MONTH - interval '1' day
    AND (CAST(FI_OUTST_AGE AS INTEGER)<=90 OR FI_OUTST_AGE IS NULL OR date_diff('day',date(date_parse(substring(cast(oldest_unpaid_bill_dt as varchar),1,8), '%Y%m%d')),date(f.dt)) <= 90)
    and exclude<>1
)
...
,CASE WHEN RejoinerMonth>=date('2022-10-01') AND RejoinerMonth<=DATE_ADD('MONTH',1,date('2022-10-01')) THEN 1 ELSE 0 END AS Fixed_PRMonth
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2022-10-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2022-10-01')
GROUP BY 1,2,3,4
)
...
where fixedmonth=date('2022-10-01') and (f_activeBOM + f_activeEOM >= 1)
