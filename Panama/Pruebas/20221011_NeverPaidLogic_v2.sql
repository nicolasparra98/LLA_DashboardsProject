...
--select distinct month, fixedmainmovement, sales_channel, count(distinct finalaccount)
...
--select distinct month, never_paid_flag,fixedmainmovement, count(distinct finalaccount)
--from FLAG_SOFT_HARD_DX
--where never_paid_flag=1
--group by 1,2,3
--order by 1,2,3
...
--select distinct month, fixedmainmovement,count(distinct F_NeverPaidFlag),count(distinct finalaccount)
--from FullTable_Adj
--where case when F_NeverPaidFlag is not null then 1 else null then numerador 
--group by 1,2
--order by 1,2

------ RESULTS QUERY -----------------------
,prueba as (select Month
, B_Final_TechFlag, B_FMCSegment, E_Final_TechFlag, E_FMCSegment, b_final_tenure,e_final_tenure,B_FixedTenure,E_FixedTenure,fixedmainmovement
...
Group by 1,2,3,4,5,6,7,8,9,B_FMCTYPE, E_FMCTYPE, First_Sales_Chnl_EOM, First_sales_CHNL_BOM, Last_Sales_CHNL_EOM, Last_Sales_CHNL_BOM , SALES_CHANNEL,sales_channel_so,fixedmainmovement
order by 1
)

select distinct month, fixedmainmovement,sum(Unique_Sales), sum(Unique_NeverPaid)
from prueba
group by 1,2
order by 2,1
