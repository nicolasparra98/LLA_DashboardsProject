...
FROM new_customers_flag F LEFT JOIN join_dx_new_customers A ON F.finalaccount = A.act_acct_cd AND F.month = A.month_start
)
...
FROM late_installation_flag F LEFT JOIN new_customer_interactions_info A ON F.finalaccount = A.act_acct_cd AND F.month = A.month_start
)
------------------------ EARLY TICKET B9 ---------------------------------
