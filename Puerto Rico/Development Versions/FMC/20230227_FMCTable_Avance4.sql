...
SELECT  *
        ...
        ,CASE   WHEN (fmc_b_att_active = 0 AND fmc_e_att_active = 1) AND ((fix_s_fla_MainMovement IN ('4.New Customer','8.Rejoiner-GrossAdd Gap') AND mob_s_fla_MainMovement = '4.New Customer') OR  (fix_s_fla_MainMovement IN ('4.New Customer','8.Rejoiner-GrossAdd Gap') AND mob_s_fla_MainMovement IS NULL) OR (fix_s_fla_MainMovement IS NULL AND mob_s_fla_MainMovement = '4.New Customer')) THEN 'Gross Adds'
                ...
                /*
                ,IF(fix_s_att_phone1 IN (SELECT DISTINCT prepaid_account FROM BOM_prepaid_convergency) OR fix_s_att_phone2 IN (SELECT DISTINCT prepaid_account FROM BOM_prepaid_convergency),IF(fix_b_fla_fmc = '3.Fixed Only','6.FMC Gap - Prepaid',IF(fix_b_fla_fmc = '2.Near FMC','6.Near FMC - Prepaid',IF(fix_b_fla_fmc = '1.Real FMC','6.Real FMC - Prepaid','FULL GAP'))),fmc_b_fla_fmc) AS fmc_b_fla_FMCadj
                ,IF(fix_s_att_phone1 IN (SELECT DISTINCT prepaid_account FROM EOM_prepaid_convergency) OR fix_s_att_phone2 IN (SELECT DISTINCT prepaid_account FROM EOM_prepaid_convergency),IF(fix_e_fla_fmc = '3.Fixed Only','6.FMC Gap - Prepaid',IF(fix_e_fla_fmc = '2.Near FMC','6.Near FMC - Prepaid',IF(fix_e_fla_fmc = '1.Real FMC','6.Real FMC - Prepaid','FULL GAP'))),fmc_e_fla_fmc) AS fmc_e_fla_FMCadj
                */
FROM FMC_base_adj
)


SELECT --fmc_b_fla_fmc,fix_b_fla_MixCodeAdj,fmc_b_fla_fmcsegment
--fmc_e_fla_fmc,fix_e_fla_MixCodeAdj,fmc_e_fla_fmcsegment
--fmc_b_fla_fmc,fmc_b_fla_tech
--fmc_e_fla_fmc,fmc_e_fla_tech
--fix_s_fla_ChurnFlag,mob_s_fla_ChurnFlag,fmc_s_fla_ChurnFlag
--fix_s_fla_Churntype,mob_s_fla_Churntype,fmc_s_fla_Churntype
---fmc_s_fla_Churntype,fmc_s_fla_ChurnFlag,fmc_s_fla_PartialTotalChurn
--fmc_s_fla_waterfall
fmc_b_fla_FMCadj,fmc_E_fla_FMCadj
,count(distinct fmc_s_att_account)
from fmc_base_adj2
--where fmc_s_fla_waterfall is null
group by 1,2--,3
order by 1,2--,3
--limit 100
