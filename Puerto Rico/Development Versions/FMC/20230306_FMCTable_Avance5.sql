...
,mobile_table AS (
SELECT *
FROM "db_stage_dev"."lcpr_postpaid_table_jan_mar06"
WHERE mob_s_dim_month = (SELECT input_month FROM parameters)
UNION ALL
SELECT *
FROM "db_stage_dev"."lcpr_prepaid_table_jan_mar06"
WHERE mob_s_dim_month = (SELECT input_month FROM parameters)
)

,convergency AS (
SELECT  *
        ,row_number() OVER (PARTITION BY fixed_account ORDER BY mobile_account desc) as row_num
        ,row_number() OVER (PARTITION BY mobile_account ORDER BY fixed_account desc) as row_num2
FROM    (
...
SELECT  IF(fix_s_dim_month IS NOT NULL,fix_s_dim_month,mob_s_dim_month) AS fmc_s_dim_month
        ...
        ,CASE   WHEN fix_b_att_active = 1 AND IF(mob_b_att_active IS NULL,0,mob_b_att_active) = 0 THEN 'Fixed Only'
                WHEN IF(fix_b_att_active IS NULL,0,fix_b_att_active) = 0 AND mob_b_att_active = 1 THEN 'Mobile Only'
                WHEN fix_b_att_active + mob_b_att_active = 2 AND mob_s_att_MobileType = 'Prepaid' THEN IF(fix_b_att_BillCode IN (SELECT DISTINCT bill_code FROM "lcpr.stage.dev"."lcpr_fix_fmc_bill_codes"),'Prepaid Real FMC','Prepaid Near FMC')
                WHEN fix_b_att_active + mob_b_att_active = 2 AND mob_s_att_MobileType = 'Postpaid' THEN IF(fix_b_att_BillCode IN (SELECT DISTINCT bill_code FROM "lcpr.stage.dev"."lcpr_fix_fmc_bill_codes"),'Postpaid Real FMC','Postpaid Near FMC')
                    ELSE NULL END AS fmc_b_fla_fmc
        ,CASE   WHEN fix_e_att_active = 1 AND IF(mob_e_att_active IS NULL,0,mob_e_att_active) = 0 THEN 'Fixed Only'
                WHEN IF(fix_e_att_active IS NULL,0,fix_e_att_active) = 0 AND mob_e_att_active = 1 THEN 'Mobile Only'
                WHEN fix_e_att_active + mob_e_att_active = 2 AND mob_s_att_MobileType = 'Prepaid' THEN IF(fix_e_att_BillCode IN (SELECT DISTINCT bill_code FROM "lcpr.stage.dev"."lcpr_fix_fmc_bill_codes"),'Prepaid Real FMC','Prepaid Near FMC')
                WHEN fix_e_att_active + mob_e_att_active = 2 AND mob_s_att_MobileType = 'Postpaid' THEN IF(fix_e_att_BillCode IN (SELECT DISTINCT bill_code FROM "lcpr.stage.dev"."lcpr_fix_fmc_bill_codes"),'Postpaid Real FMC','Postpaid Near FMC')
                    ELSE NULL END AS fmc_e_fla_fmc
FROM    (
        SELECT A.*, B.mobile_account
        FROM fixed_table A LEFT JOIN (SELECT * FROM convergency WHERE row_num = 1) B -- AND row_num2 = 1) B
...
,FMC_base_adj AS (
SELECT  *
        ,IF(fmc_b_fla_fmc = 'Fixed Only',CONCAT(fix_b_fla_MixCodeAdj,' Fixed'),IF(fmc_b_fla_fmc = 'Mobile Only','P1 Mobile',CONCAT(CAST((CAST(SUBSTR(fix_b_fla_MixCodeAdj,1,1) AS int) + 1) AS VARCHAR),'P FMC'))) AS fmc_b_fla_FMCsegment
        ,IF(fmc_e_fla_fmc = 'Fixed Only',CONCAT(fix_e_fla_MixCodeAdj,' Fixed'),IF(fmc_e_fla_fmc = 'Mobile Only','P1 Mobile',CONCAT(CAST((CAST(SUBSTR(fix_e_fla_MixCodeAdj,1,1) AS int) + 1) AS VARCHAR),'P FMC'))) AS fmc_e_fla_FMCsegment
...
SELECT fmc_b_fla_fmc, count(distinct fmc_s_att_account)
FROM fmc_base_adj2
--where fmc_b_fla_fmc is null and fmc_e_fla_fmc is null
group by 1
order by 1
