...
WHERE TRIM(order_type) = 'RELOCATION/TRAN'
    AND account_type = 'RES'
    AND order_status = 'COMPLETE'
    AND DATE_TRUNC('MONTH',DATE(completed_date)) = (SELECT input_month FROM parameters)
)
...
SELECT  fix_s_dim_month
        ,fix_s_att_account
        ,fix_b_att_active
        ,IF(fix_s_fla_ChurnFlag = '1. Fixed Churner',0,fix_e_att_active) AS fix_e_att_active
        ,fix_s_att_phone1
        ...
        ,fix_e_fla_tech
        ,fix_e_fla_fmc
        ,IF(fix_s_fla_ChurnFlag = '1. Fixed Churner',0,fix_e_mes_numRGUS) AS fix_e_mes_numRGUS
        ,fix_e_fla_MixNameAdj
        ,fix_e_fla_MixCodeAdj
        ,fix_e_fla_BB
        ,fix_e_fla_TV
        ...
        ,fix_e_fla_subsidized
        ,fix_e_att_BillCode
        ,IF(fix_s_fla_ChurnFlag = '1. Fixed Churner','6.Null last day',fix_s_fla_MainMovement) AS fix_s_fla_MainMovement
        ,IF(fix_s_fla_ChurnFlag = '1. Fixed Churner','3. No Spin',fix_s_fla_SpinMovement) AS fix_s_fla_SpinMovement
        ...
        ,CASE   WHEN fix_s_att_account IN (SELECT distinct account_id FROM transfers) /*AND fix_s_fla_MainMovement = '6.Null last day'*/ THEN '3. Fixed Transfer'
        ...
        ,CASE   WHEN fix_s_att_account IN (SELECT distinct account_id FROM transfers) /*AND fix_s_fla_MainMovement = '6.Null last day'*/ THEN 'Transfer'
...
/*
select DISTINCT order_type,cease_reason_desc, COUNT(DISTINCT ACCOUNT_ID)
from "lcpr.stage.prod"."so_hdr_lcpr" 
where account_id in (select distinct fix_s_att_account from final_fixed_flags
where fix_s_fla_MainMovement = '6.Null last day' AND fix_s_fla_ChurnType <> '2. Fixed Involuntary Churner')
GROUP BY 1,2
*/
