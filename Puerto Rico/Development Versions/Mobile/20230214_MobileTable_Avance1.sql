...
,disconnections AS (
SELECT  date_trunc('month',date(substr(lst_susp_dt,1,10))) AS month
        ,cust_id AS account
        ,date(substr(lst_susp_dt,1,10)) AS disconnection_date
        ,lst_susp_rsn_desc 
        ,IF(lower(lst_susp_rsn_desc) LIKE '%no%pay%','Involuntary','Voluntary') AS churn_type
from "lcpr.stage.dev"."tbl_pstpd_cust_mstr_ss_data" 
where substr(lst_susp_dt,1,10) like '20%'
    AND date_trunc('month',date(substr(lst_susp_dt,1,10))) = (SELECT input_month FROM parameters)
    AND cust_sts = 'O'
    AND cust_type = 'Liberty Regular'
    AND acct_type_cd = 'I'
    AND rgn_nm = 'PR'
...
SELECT  month
        ...
        ,churn_type
FROM    (SELECT *
                ,row_number() over (PARTITION BY account order by disconnection_date DESC) as row_num
        FROM disconnections)
where row_num = 1
)
...
SELECT  A.*
        ,CASE   WHEN B.account IS NOT NULL THEN '1. Mobile Churner'
                WHEN B.account IS NULL THEN '2. Mobile NonChurner'
...
FROM spin_movement_flag A LEFT JOIN all_churners B ON A.mob_s_att_account = B.account AND A.mob_s_dim_month = B.month
)


select distinct mob_s_dim_month,mob_b_att_active,mob_e_att_active, mob_s_fla_MainMovement,mob_s_fla_SpinMovement
        ,mob_s_fla_ChurnFlag,mob_s_fla_ChurnType
        , count(distinct mob_s_att_account) as accounts, sum(mob_b_mes_numRGUS) as BOM_RGUs,sum(mob_e_mes_numRGUS) as BOM_RGUs
