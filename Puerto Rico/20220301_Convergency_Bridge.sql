...
select  distinct *
        ...
        ,row_number() over (partition by mobile_account order by fixed_account desc) as row_num_mobile
        ,CASE WHEN fixed_account IS NOT NULL AND MOBILE_ACCOUNT IS NULL THEN 'Fixed Only'
            WHEN fixed_account IS NOT NULL AND MOBILE_ACCOUNT IS NOT NULL AND Convergency = 'Postpaid' then if(bill_code IN (SELECT DISTINCT bill_code FROM "lcpr.stage.dev"."lcpr_fix_fmc_bill_codes"),'Real FMC','Near FMC')
            when fixed_account IS NOT NULL AND MOBILE_ACCOUNT IS NOT NULL AND Convergency = 'Prepaid' then 'FMC Prepaid' end as FMC_Flag_Oval
from(
SELECT A.*,B.postpaid_account AS MOBILE_ACCOUNT, 'Postpaid' as convergency 
FROM fixed_base A INNER JOIN POSTPAID B ON fixed_phone1 = postpaid_account 
UNION ALL
SELECT A.*,B.postpaid_account AS MOBILE_ACCOUNT, 'Postpaid' as convergency 
FROM fixed_base A INNER JOIN POSTPAID B ON fixed_phone2 = postpaid_account 
UNION ALL
SELECT A.*,B.prepaid_account AS MOBILE_ACCOUNT, 'Prepaid' as convergency 
FROM fixed_base A INNER JOIN PREPAID B ON fixed_phone1 = prepaid_account 
UNION ALL
SELECT A.*,B.prepaid_account AS MOBILE_ACCOUNT, 'Prepaid' as convergency 
FROM fixed_base A INNER JOIN PREPAID B ON fixed_phone2 = prepaid_account 
UNION ALL
...
order by 1,2,3
*/
select row_num_mobile, count(distinct fixed_account)
from second_step
--where convergency = 'Postpaid'
group by 1--,2
order by 1--,2 asc
