...
FmcTable AS (
SELECT DISTINCT * FROM "lla_cco_int_san"."cr_fmc_table_nov16_np"
)
...
,Last_Interaction as (
  ...
  first_value(interaction_start_time) over(partition by account_id,date_trunc('Month',date(interaction_start_time)) order by interaction_start_time desc) as last_interaction_date
  From (select * from prueba having row_num = 1)
  WHERE (account_type='RESIDENCIAL' or account_type='PROGRAMA HOGARES CONECTADOS') and date_trunc('Month',interaction_start_time)>=date('2022-01-01') and interaction_status <> 'ANULADA'
