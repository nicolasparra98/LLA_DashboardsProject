...
--CREATE TABLE IF NOT EXISTS "lla_cco_int_san"."cr_fixed_table_DEC15"  AS  
...
,CustomerBase_BOM AS(
SELECT DISTINCT DATE_TRUNC('MONTH', DATE_ADD('MONTH', 1, DATE(dt))) AS Month, act_acct_cd AS AccountBOM,dt AS b_date,act_contact_phone_1 as b_phone,
...
,CustomerBase_EOM AS(
SELECT DISTINCT DATE_TRUNC('MONTH', DATE(dt)) AS Month, dt as e_date, act_acct_cd as AccountEOM, act_contact_phone_1 as e_phone, pd_vo_prod_nm as e_vo_nm, 
...
*/  
    FROM mora_useful_fields c 
    WHERE date(dt) = (date_trunc('MONTH', DATE(dt)) + interval '1' MONTH - interval '1' day) --and DATE_TRUNC('Month', date(dt))<>date('2022-10-01')) OR dt='2022-10-01' OR dt='2022-12-02') 
    and (mora_fix <= 90 or mora_fix is null)
)
